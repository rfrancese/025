...
/*        Snake snake = world.snake;
    ...
    private void disegnaTarget(ArrayList<Target> listaTarget, Graphics g) {
		...
		for(int i=0; i<lista.size(); i++){
			...
			g.drawPixmap(tmp.getSfondo(), tmp.getX(), tmp.getY());
			
			 int numW = tmp.getImmagineNumero().getWidth()/2;
		     int numH = tmp.getImmagineNumero().getHeight()/2;
		     
		     g.drawPixmap(tmp.getImmagineNumero(), tmp.getX()+(tmp.getSfondo().getWidth()/2)-numW, tmp.getY()+(tmp.getSfondo().getHeight()/2)-numH);
		}
	}
